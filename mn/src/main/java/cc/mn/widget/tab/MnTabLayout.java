...
public class MnTabLayout extends RelativeLayout {
    ...
    public MnTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.TabLayoutStyleable);
        //是否是选中状态
        isChecked = ta.getBoolean(R.styleable.TabLayoutStyleable_tabChecked, false);
        //标题的颜色，内容
        titleColor = ta.getColor(R.styleable.TabLayoutStyleable_tabTitleColor, Color.GRAY);
        titleText = ta.getString(R.styleable.TabLayoutStyleable_tabTitle);
        //图标图片
        tabIcon = ta.getDrawable(R.styleable.TabLayoutStyleable_tabIcon);
        tabIconOver = ta.getDrawable(R.styleable.TabLayoutStyleable_tabIconOver);
        ta.recycle();
